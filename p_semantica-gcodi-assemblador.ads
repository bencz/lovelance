...
private
    ...
    type tipus_insass is (movl, addl,  leal,  pushl, popl,
                          xorl, orl,   andl,  notl,  subl,
                          sarl, idivl, imull, jmp,   cmpl,
                          jge,  jg,    jl,    jle,   jne,
                          je,   call,  ret,   etiq);

    type tipus_registre is (eax, ebx, ecx, edx,
                            esi, edi, ebp, esp);
    ...
    type registre (a: tipus_adressament := directe) is
        record
            case a is
                when immediat | 
                     absolut         => x  : integer;
    ...
    type ins_ass (ti : tipus_insass := movl) is
        record
            case ti is
                when movl  | 
                     leal  | 
